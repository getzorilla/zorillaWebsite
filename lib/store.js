...
// People, by username or display name. The marketplace is somebody's shelf as
// much as it is a pile of files, so a name typed into the box has to find them.
export async function searchProfiles(query, limit = 8) {
  const term = String(query ?? '').trim().toLowerCase().replace(/^@/, '')
  if (!term) return []
  const all = await listProfiles()
  return all
    .filter((p) => `${p.handle} ${p.name ?? ''}`.toLowerCase().includes(term))
    .slice(0, limit)
}
...
export async function listListings({ kind = null, author = null, query = '', viewer = null } = {}) {
  ...
  const found = [...published, ...SHIPPED]
    ...
    // searching for somebody by name should find their work, not just work
    // whose title happens to contain their name
    .filter((l) => !term || `${l.title} ${l.summary} ${l.slug} ${l.authorHandle}`.toLowerCase().includes(term))
  ...
  const authors = await authorsFor(found.map((l) => l.authorHandle))
  return found.map((l) => ({
    ...l,
    likes: counts[l.slug] ?? 0,
    liked: mine.has(l.slug),
    author: authors.get(l.authorHandle) ?? { handle: l.authorHandle, name: '', avatar: null },
  }))
}

// A listing shows who made it, so the profiles are fetched once for the whole
// page rather than per row.
async function authorsFor(handles) {
  const store = await backend()
  const wanted = new Set(handles.filter(Boolean))
  if (!wanted.size) return new Map()
  const all = await store.profiles()
  return new Map(all
    .filter((p) => wanted.has(p.handle))
    .map((p) => [p.handle, { handle: p.handle, name: p.name ?? '', avatar: p.avatar ?? null }]))
}

export async function getListing(slug) {
  const store = await backend()
  const found = (await store.listing(slug)) ?? SHIPPED.find((l) => l.slug === slug) ?? null
  if (!found) return null
  const authors = await authorsFor([found.authorHandle])
  return { ...found, author: authors.get(found.authorHandle) ?? { handle: found.authorHandle, name: '', avatar: null } }
}
...
// Posts belong to whoever runs the site, so there is no author field and no
// permission to work out: the API refuses anybody who is not the owner, and
// everything stored here is already public.
export async function listPosts({ drafts = false } = {}) {
  const store = await backend()
  const all = await store.postList()
  return drafts ? all : all.filter((post) => !post.draft)
}

export async function getPost(slug) {
  const store = await backend()
  return store.post(String(slug ?? ''))
}

export async function savePost(post) {
  const store = await backend()
  const slug = String(post.slug ?? '')
  if (!slug) throw new Error('A post needs a title.')
  const existing = await store.post(slug)
  return store.putPost({
    createdAt: existing?.createdAt ?? new Date().toISOString(),
    images: [],
    summary: '',
    draft: false,
...
export async function removePost(slug) {
  const store = await backend()
  await store.dropPost(String(slug))
  return { removed: 1 }
}
